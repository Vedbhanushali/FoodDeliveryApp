...
and if we want to use `JS statement` in JSX, we have to wrap in
...
It takes little time for browser to process the object to make it readable in json format so it is asynchronous process so we have to add await for this function.
